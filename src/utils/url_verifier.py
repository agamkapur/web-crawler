...
def protocol_checks(url: str) -> bool:
    ...
    try:
        ...
        req = Request(url, headers={'User-Agent': 'Mozilla/5.0 (compatible; MyCrawler/1.0)'})
...
def operational_checks(url: str) -> bool:
    ...
    try:
        ...
        try:
            req = Request(robots_url, headers={'User-Agent': 'Mozilla/5.0 (compatible; MyCrawler/1.0)'})
            with urlopen(req, timeout=5) as response:
                if response.status == 200:
                    print(f"Operational check: robots.txt found and accessible")
        except:
            print(f"Operational check: robots.txt not accessible")
        
        # Check for rate limiting headers
        req = Request(url, headers={'User-Agent': 'Mozilla/5.0 (compatible; MyCrawler/1.0)'})
        with urlopen(req, timeout=10) as response:
            if 'Retry-After' in response.headers:
                print(f"Operational check: Rate limiting detected (Retry-After: {response.headers['Retry-After']})")
                return False
            
            if response.status == 403:
                print(f"Operational check: Access forbidden (403)")
                return False
...
def security_checks(url: str) -> bool:
    ...
    try:
        ...
        private_patterns = ['192.168.', '10.', '172.16.', '172.17.', '172.18.', '172.19.', '172.20.', '172.21.', '172.22.', '172.23.', '172.24.', '172.25.', '172.26.', '172.27.', '172.28.', '172.29.', '172.30.', '172.31.']
