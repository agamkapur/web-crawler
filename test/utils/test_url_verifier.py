...
class TestURLVerifier(unittest.TestCase):
    ...
    def test_syntactic_checks_empty_url(self):
        ...
        for url in empty_urls:
            with self.subTest(url=repr(url)):
                self.assertFalse(
                    syntactic_checks(url), f"Empty URL should be invalid: {repr(url)}"
                )
    ...
    def test_syntactic_checks_whitespace_urls(self):
        ...
        for url in whitespace_urls:
            with self.subTest(url=repr(url)):
                self.assertFalse(
                    syntactic_checks(url),
                    f"URL with whitespace should be invalid: {repr(url)}",
                )
    ...
    def test_syntactic_checks_invalid_schemes(self):
        ...
        for url in invalid_scheme_urls:
            with self.subTest(url=url):
                self.assertFalse(
                    syntactic_checks(url),
                    f"URL with invalid scheme should be invalid: {url}",
                )

    def test_syntactic_checks_missing_domain(self):
        """Test syntactic_checks with URLs missing domain"""
        invalid_domain_urls = ["https://", "http://", "https:///path"]

        for url in invalid_domain_urls:
            with self.subTest(url=url):
                self.assertFalse(
                    syntactic_checks(url),
                    f"URL without domain should be invalid: {url}",
                )
    ...
    def test_syntactic_checks_invalid_ports(self):
        ...
        for url in invalid_port_urls:
            with self.subTest(url=url):
                self.assertFalse(
                    syntactic_checks(url),
                    f"URL with invalid port should be invalid: {url}",
                )
    ...
    def test_syntactic_checks_valid_ports(self):
        ...
        for url in valid_port_urls:
            with self.subTest(url=url):
                self.assertTrue(
                    syntactic_checks(url), f"URL with valid port should be valid: {url}"
                )
    ...
    def test_is_valid_domain_valid_domains(self):
        ...
        for domain in valid_domains:
            with self.subTest(domain=domain):
                self.assertTrue(
                    is_valid_domain(domain), f"Domain should be valid: {domain}"
                )
    ...
    def test_is_valid_domain_invalid_domains(self):
        ...
        for domain in invalid_domains:
            with self.subTest(domain=domain):
                self.assertFalse(
                    is_valid_domain(domain), f"Domain should be invalid: {domain}"
                )
    ...
    def test_is_valid_path_query_valid(self):
        ...
        for path, query in valid_combinations:
            with self.subTest(path=path, query=query):
                self.assertTrue(
                    is_valid_path_query(path, query),
    ...
    @patch("utils.url_verifier.semantic_checks")
    @patch("utils.url_verifier.syntactic_checks")
    def test_verify_syntactic_fails(
        self,
        mock_syntactic,
        mock_semantic,
        mock_protocol,
    ...
    @patch("utils.url_verifier.semantic_checks")
    @patch("utils.url_verifier.syntactic_checks")
    def test_verify_semantic_fails(
        self,
        mock_syntactic,
        mock_semantic,
        mock_protocol,
    ...
    def test_verify_exception_handling(self):
        ...
        with patch(
            "utils.url_verifier.syntactic_checks",
    ...
    def test_verify_function_consistency(self):
        """
        Test that verify function returns same result as
        syntactic_checks for basic cases
        """
        test_urls = [
            "https://example.com",
        ...
        for url in test_urls:
            with self.subTest(url=url):
                ...
                # We can't easily test the full verify function without
    ...
    def test_edge_cases(self):
        ...
        for url in edge_cases:
            with self.subTest(url=url):
                ...
                # We expect most to be valid, but some might fail due to
                # strict validation
